...
Import and Re-import use the common score import service. Users select MusicXML/MXL or uncompressed NotePan schema 6/8, optional WAV/Ogg audio, and a MusicXML-only overlay. NotePan disables overlay and written-octave controls. Re-import requires a selected song, retains its stable song ID, and creates a new immutable import version. Failures and persistent non-blocking warnings display their code, explanation, and remediation in the details area.

Delete is a two-step confirmation. The preview names `packages/<song_id>` and states that external score/audio sources are unchanged. Confirmation first removes the song from the atomic index, making it invisible, then removes all cached import versions. A cache cleanup failure is retained as a warning and never restores a stale Library entry.
