...
Each result is rebuilt from Phase 1 Judgement Records using the identified score-rule document. Stored identity includes song, importer, chart, Instrument Profile, judgement-rule, and score-rule versions. New sessions also retain the selected practice-tempo ID and multiplier; older results display this field as not recorded. Timing median and EARLY / ON TIME / LATE counts use only `outcome=judged` records with numeric `delta_us`; Miss retains null timestamps, and Extra Hit plus wrong target/technique remain separate counts.
