...
func _initialize() -> void:
	...
	for index: int in 64:
		notes.append({"note_id":"note-%d" % index,"timestamp_us":2_000_000,"technique":["tone","ding","slap"][index % 3],"target_id":["tone-1","ding","outer-hit-radius"][index % 3],"hand":"right" if index % 2 == 0 else "left"})
	...
	if built.get("ok",false):
		...
		very_fast.update(1_500_000)
		_check(very_fast.active_count() == 64 and very_fast.lookahead_us() == 500_000, "very fast speed activates the same notes half a second before judgement", failures)
		...
		var overflow_scheduler := Scheduler.new(overflow_built["chart"], profile, 64); overflow_scheduler.update(1_000_000)
		_check(overflow_scheduler.overflow_count == 1, "pool overflow is observable", failures)
	var quality := View.visual_quality_contract()
	_check(View.hand_color("right") == Color("38d9ff") and View.hand_color("left") == Color("ff72b6") and View.hand_color("unspecified", Color("123456")) == Color("123456"), "explicit right and left hands select distinct colors with a legacy fallback", failures)
	_check(quality["procedural_field_shader"] and quality["audio_time_driven_shader"] and quality["deterministic_shader"] and quality["note_bloom_shader"] == "integrated_sdf_gaussian" and quality["tone_note_shape"] == "foreground_emissive_orb" and quality["orb_shading"] == "saturated_cyan_center_hot_gradient" and quality["orb_draw_order"] == "above_handpan_and_targets" and quality["orb_bloom_strength"] == "strong_atmospheric_spill" and quality["bloom_profile"] == "luminous_core_diffused_mist_atmospheric_spill" and quality["bloom_shader_capacity"] == 16 and quality["normal_glow_layers"] == 1 and quality["smooth_bloom_falloff"] == "continuous_gaussian" and not quality["single_note_ring"] and not quality["spawn_ring_pulse"] and not quality["hollow_note_core"] and not quality["black_note_core"] and not quality["white_impact_fill"] and not quality["impact_rays"] and quality["bloom_strength"] == "pronounced" and quality["hit_bloom_strength"] == "bright_surge" and quality["dense_load_threshold"] == 16 and quality["dense_load_halo_layers"] == 1, "foreground tone notes use saturated cyan center-hot gradients without a spawn-ring pulse", failures)
	...
	_check(not quality["note_trails"] and quality["technique_palette"].size() == 3 and quality["hand_palette"] == {"right":"right_hand_luminous","left":"left_hand_luminous"} and quality["reference_window"] == [1600,900] and quality["launch_mode"] == "maximized", "hand-colored trail-free luminous palette and maximized window contract", failures)
	...
	var main_scene := FileAccess.get_file_as_string("res://presentation/main.tscn")
	_check(main_scene.contains("visible = false") and main_source.contains("set_process_unhandled_input(false); _open_product_flow.call_deferred(); return") and main_source.contains("# Explicit automation/gameplay launches opt in to visibility here.\n\tvisible = true"), "startup gameplay remains hidden while Product Flow is added at the safe deferred point", failures)
	var project_settings := FileAccess.get_file_as_string("res://project.godot")
	_check(project_settings.contains("environment/defaults/default_clear_color=Color(0.0117647, 0.0196078, 0.0627451, 1)"), "pre-menu startup frame uses the shared deep-indigo background", failures)
	...
	_check(NoteScrollSpeeds.preset("normal")["lookahead_us"] == 1_000_000 and NoteScrollSpeeds.preset("normal")["multiplier"] == 1.0, "the former 2.0x visual speed is the normal baseline", failures)
