...
func _initialize() -> void:
	...
	_check(package.get("schema_version") == "1.3.0" and package.get("handpan_scale_name") == "D Kurd 9" and package.get("performance_layout", {}).get("slots", []).size() == 3 and not chart.has("handpan_scale_name"), "handpan scale and performance layout remain package metadata rather than Runtime Chart data", failures)
	...
	var effective_profile := PerformanceLayouts.effective_profile(profile, package["performance_layout"])
	var runtime_result := ChartFactory.build(chart, effective_profile, int(package["duration_us"])); _check(runtime_result.get("ok"), "imported chart enters existing Runtime Chart factory", failures)
	...
	var scheduler := Scheduler.new(runtime_result["chart"], effective_profile, 64); scheduler.update(0); var active: Array[Dictionary] = []
	for slot: Dictionary in scheduler.active_slots():
		if slot["active"]: active.append(slot["note"])
	_check(not active.is_empty() and active[0]["target_id"] == "tone-1", "existing radial scheduler uses song-specific target mapping", failures)
	...
	var metadata := {"song_id":package["song_id"], "importer_version":package["importer_version"], "chart_version":package["chart_schema_version"], "profile_id":package["profile_id"], "performance_layout_id":package["performance_layout"]["layout_id"], "judgement_rule_id":judgement_rules["rule_id"], "score_rule_id":score_rules["rule_id"]}
