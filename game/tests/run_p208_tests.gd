...
func _initialize() -> void:
	...
	var index := {"schema_version":"1.0.0", "songs":[_entry("z-song", "Zulu"), _entry("broken", "Broken"), _entry("a-song", "alpha")]}
	repositories.songs.save(index)
	var query := service.query(repository_root, repositories.songs, "profile-a")
	_check(query.get("ok") and query["songs"].size() == 3, "broken song does not prevent library query", failures)
	_check(query["songs"][0]["song_id"] == "a-song" and query["songs"][1]["song_id"] == "broken" and query["songs"][2]["song_id"] == "z-song", "library ordering is deterministic title then ID", failures)
	...
	var zulu: Dictionary = query["songs"][2]
	...
	_check(service.query(repository_root, repositories.songs, "profile-a")["songs"].size() == 2, "delete updates atomic index", failures)
	...
	files.remove_tree(root); _finish(failures, 12)

func _make_package(files: RefCounted, root: String, song_id: String, title: String, profile_id: String, assets: bool) -> void:
	var relative := "packages/%s/v1-cache" % song_id; var path := root.path_join(relative); DirAccess.make_dir_recursive_absolute(path)
	var package := {"schema_version":"1.0.0", "song_id":song_id, "import_version":1, "title":title, "artist":"Artist", "duration_us":1250000, "chart_schema_version":"1.0.0", "artwork_path":"", "profile_id":profile_id, "chart_path":"chart.json", "audio":{"runtime_path":"runtime.ogg"}}
	files.write_text(path.path_join("package.json"), Canonical.encode(package) + "\n")
	if assets: files.write_text(path.path_join("chart.json"), "{}\n"); files.write_text(path.path_join("runtime.ogg"), "fixture")
