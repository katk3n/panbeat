...
func _initialize_gameplay(imported_package_path: String = "") -> void:
	...
	if _imported_product_session:
		var persisted: Dictionary = Repositories.new(_repository_root_override).settings.load()
...
func _open_completed_results() -> void:
	visible = false; set_process(false); set_process_unhandled_input(false); get_tree().root.add_child(ResultsView.new())
