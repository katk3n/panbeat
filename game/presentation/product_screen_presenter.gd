...
static func result_sections(record: Dictionary) -> Dictionary:
	...
	return {
		...
		"technical":"Result %s · importer %s · chart %s · profile %s\njudgement %s · score %s · practice tempo %s · identity errors T%d / K%d / N%d" % [record.get("result_id", "unknown"), metadata.get("importer_version", "?"), metadata.get("chart_version", "?"), metadata.get("profile_id", "?"), metadata.get("judgement_rule_id", "?"), metadata.get("score_rule_id", "?"), "%.0f%%" % (float(metadata["practice_tempo_multiplier"]) * 100.0) if metadata.get("practice_tempo_multiplier") is float else "not recorded", int(errors.get("wrong_target", 0)), int(errors.get("wrong_technique", 0)), int(errors.get("no_candidate", 0))]
	}
