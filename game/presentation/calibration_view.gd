...
func _ready() -> void:
	set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT); _diagnostics_output = _argument("--calibration-diagnostics-output=")
	...
	_build_ui(); _load_saved(); _adapter = Adapter.new(); _adapter.profile = _load_json("res://config/default-instrument-profile.json"); _adapter.record_received.connect(_on_record); add_child(_adapter); _refresh_midi_status.call_deferred()
...
func _process(_delta: float) -> void:
	...
	if _cues_emitted < TARGET_CUES and now >= _next_cue_us:
		...
		_running = false; _status.text = "CAPTURE COMPLETE — Analyze samples. Miss and Extra Hit are excluded."
...
func _build_ui() -> void:
	...
	var margin := MarginContainer.new(); margin.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT); for key: String in ["margin_left", "margin_right", "margin_top", "margin_bottom"]: margin.add_theme_constant_override(key, 42)
	add_child(margin); var layout := VBoxContainer.new(); layout.add_theme_constant_override("separation", 12); margin.add_child(layout)
	var title := Label.new(); title.text = "CALIBRATION"; title.add_theme_font_size_override("font_size", 30); layout.add_child(title)
	...
	_status = Label.new(); _status.text = "READY — Start when Mood Pan input is connected."; _status.add_theme_font_size_override("font_size", 21); layout.add_child(_status)
	...
	_samples_label = RichTextLabel.new(); _samples_label.fit_content = true; _samples_label.custom_minimum_size.y = 230; _samples_label.text = "No samples yet."; layout.add_child(_samples_label); start.grab_focus()

func _start() -> void:
	_samples = []; _proposal = {}; _pending_stimulus_us = -1; _early_input_us = -1; _early_technique = ""; _cues_emitted = 0; _running = true; _next_cue_us = Time.get_ticks_usec() + 1_000_000; _status.text = "GET READY — First cue in one second."; _refresh_samples()
...
func _analyze() -> void:
	...
	_refresh_samples()
	if not _proposal.get("ok", false): _status.text = "RETRY — %s" % _proposal.get("message", "Calibration failed."); return
	...
	_status.text = "PASS — PROPOSAL READY: median %+.1f ms, MAD %.1f ms, %d outlier(s) excluded. Review/fine-tune, then Apply & Save." % [int(_proposal["median_delta_us"]) / 1000.0, int(_proposal["mad_us"]) / 1000.0, int(_proposal.get("outlier_count", 0))]
